...
from aoc_python.utils import Point, load_stripped_lines, clear_outputs

from tqdm import tqdm


def parse(lines: list[str]) -> Iterable[Point]:
    for char in lines[0]:
        if char == ">":
            yield Point(1, 0)
        elif char == "<":
...
TILES: list[tuple[Point, ...]] = [
    ...
    (Point(0, 0), Point(1, 0), Point(2, 0), Point(2, 1), Point(2, 2)),  # _|
    (Point(0, 0), Point(0, 1), Point(0, 2), Point(0, 3)),  # |
    (Point(0, 0), Point(0, 1), Point(1, 1), Point(1, 0)),  # []
]


def generate_tiles(limit: int) -> Iterable[tuple[Point, ...]]:
    i = 0
    while i < limit:
        yield TILES[i % 5]
        i += 1
...
def move_tile(tile: tuple[Point, ...], direction: Point) -> tuple[Point, ...]:
    return tuple(p + direction for p in tile)


def is_colliding(tile: tuple[Point, ...], rocks: set[Point]) -> bool:
...
def print_state(tile: tuple[Point, ...], rocks: set[Point]) -> None:
...
def get_rock_heights(rocks: set[Point]) -> tuple[int, ...]:
...
def simulate_tetris(
    directions_gen: Iterator[tuple[Point, bool]],
    tile_limit: int,
) -> int:
    highest_rock_y = 0
    rocks: set[Point] = set()
    ...
    for iteration, tile in tqdm(enumerate(generate_tiles(tile_limit)), total=tile_limit):
        ...
        falling_tile = move_tile(tile, Point(2, 4 + highest_rock_y))
        falling_step = 0
        while True:
            ...
            lowered_tile = move_tile(falling_tile, Point(0, -1))
...
#             mh = min(heights)
#             height_differences = tuple([h - mh for h in heights])
#             mem_key = (tuple(p.x for p in tile), falling_step, height_differences)
#             mem_map[mem_key].append((iteration, max(heights)))
#             print(dict(mem_map))
#         shifted_tile = move_tile(tile, direction)
#         if not is_colliding(shifted_tile, rocks):
#             tile = shifted_tile
#         lowered_tile = move_tile(tile, Point(0, -1))
#         if is_colliding(lowered_tile, rocks):
#             return tile
#         tile = lowered_tile
#         falling_step += 1
...
def directions_generator(directions: Iterable[Point]) -> Iterable[tuple[Point, bool]]:
...
def test_parse() -> None:
    ...
    assert directions[0] == Point(1, 0)
    assert directions[3] == Point(-1, 0)
    assert directions[-1] == Point(1, 0)
