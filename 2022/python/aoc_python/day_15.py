...
def parse(lines: list[str]) -> dict[Point2, Point2]:
    sensor_data: dict[Point2, Point2] = {}
    ...
    for line in lines:
        ...
        sensor_data[Point2(int(sensor_coordinates[0]), int(sensor_coordinates[1]))] = Point2(
            int(beacon_coordinates[0]), int(beacon_coordinates[1])
        )
    return sensor_data


def manhattan_distance(a: Point2, b: Point2) -> int:
    return abs(a.x - b.x) + abs(a.y - b.y)


def is_covered_by_any_sensor(p: Point2, distances: dict[Point2, int]) -> bool:
...
def covered_interval_for_sensor(sensor: Point2, target_y: int, distance: int) -> tuple[Point2, Point2] | None:
    p = Point2(sensor.x, target_y)
    y_dist = manhattan_distance(sensor, p)
    x_dist = distance - y_dist
    if x_dist < 0:
        return None
    return Point2(p.x - x_dist, target_y), Point2(p.x + x_dist, target_y)
...
def count_covered_points(intervals: list[tuple[Point2, Point2]], beacons: set[Point2], target_y: int) -> int:
    points = sorted(
    ...
    first_opened_point: Point2 = Point2(0, 0)
...
def extract_lines_for_sensor(center: Point2, size: int) -> list[Line]:
    p1 = Point2(center.x - size, center.y)
    p2 = Point2(center.x + size, center.y)

    return [Line(1, p1.y - p1.x), Line(-1, p1.y + p1.x), Line(1, p2.y - p2.x), Line(-1, p2.y + p2.x)]


def intersect_lines(a: Line, b: Line) -> Point2 | None:
    if a.slope == b.slope:
        return None
...
def intersect_sensors(center_a: Point2, dist_a: int, center_b: Point2, dist_b: int) -> tuple[Point2, ...]:
...
def is_point_covered_by_sensors(p: Point2, sensors: dict[Point2, int]) -> bool:
...
def test_parse() -> None:
    ...
    assert sensor_data[Point2(2, 18)] == Point2(-2, 15)
    assert sensor_data[Point2(20, 14)] == Point2(25, 17)


def test_manhattan() -> None:
    assert manhattan_distance(Point2(0, 0), Point2(0, 5)) == 5
    assert manhattan_distance(Point2(0, 0), Point2(5, 5)) == 10


def test_lines() -> None:
    assert set(extract_lines_for_sensor(Point2(1, 0), 1)) == {Line(-1, 0), Line(1, 0), Line(1, -2), Line(-1, 2)}
    assert set(extract_lines_for_sensor(Point2(-2, -2), 2)) == {Line(1, 2), Line(1, -2), Line(-1, -2), Line(-1, -6)}


def test_intersect() -> None:
    assert intersect_lines(Line(1, 123), Line(1, -123)) == None
    assert intersect_lines(Line(1, -1), Line(-1, 1)) == Point2(1, 0)
    assert intersect_lines(Line(-1, -2), Line(1, 0)) == Point2(-1, -1)
